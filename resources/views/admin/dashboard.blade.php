...
<div class="mb-6 flex flex-col gap-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div
            ...
            <div class="flex items-center justify-between mb-4">
                <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">Modal Usaha</span>
            ...
            <div class="flex items-baseline gap-2">
                <span class="text-2xl font-bold text-gray-900">
                    Rp {{ number_format($modal_usaha, 0, ',', '.') }}
                </span>
            </div>
            <div class="mt-2 text-xs font-medium text-violet-600 flex items-center gap-1">
                <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
                        d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h10a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H8a2 2 0 01-2-2v-4zm6 4a2 2 0 100-4 2 2 0 000 4z"
                        clip-rule="evenodd" />
                </svg>
                <span>Total modal yang diinvestasikan</span>
            </div>
        </div>
        ...
        <div
            ...
            <div class="mt-2 text-xs font-medium text-rose-600 flex items-center gap-1">
                <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
        ...
        <div
            ...
            <div class="flex items-baseline gap-2">
                <span class="text-2xl font-bold {{ $laba_bersih >= 0 ? 'text-gray-900' : 'text-rose-600' }}">
                    Rp {{ number_format($laba_bersih, 0, ',', '.') }}
                </span>
            </div>
            <div
                class="mt-2 text-xs font-medium {{ $laba_bersih >= 0 ? 'text-emerald-600' : 'text-rose-600' }} flex items-center gap-1">
                <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
                        d="M12 7a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0V8.414l-4.293 4.293a1 1 0 01-1.414 0L8 10.414l-4.293 4.293a1 1 0 01-1.414-1.414l5-5a1 1 0 011.414 0L11 10.586 14.586 7H12z"
                        clip-rule="evenodd" />
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm shadow-gray-200/20 lg:col-span-2">
            ...
            <div class="h-64 flex items-end justify-between relative mt-4">
                ...
                <!-- Label X-Axis -->
                <div
                    class="absolute inset-x-0 bottom-[-24px] flex justify-between text-[10px] text-gray-400 font-semibold">
        ...
        {{-- Quick Actions --}}
        <div
            class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm shadow-gray-200/20 flex flex-col justify-between">
            <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <a href="{{ route('admin.transactions.create') }}"
                        class="flex flex-col items-center justify-center p-4 rounded-xl border border-gray-100 hover:border-emerald-500 hover:bg-emerald-50/10 group transition-all duration-200 text-center">
                        <span
                            class="p-2 bg-emerald-50 text-emerald-600 rounded-lg group-hover:bg-emerald-500 group-hover:text-white transition-colors mb-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                                ...
                                <td class="px-6 py-4 font-semibold text-gray-400">
                                    <div class="flex items-center gap-2">
                                        <div class="flex flex-col">
                                            <span
                                                class="font-semibold text-gray-800 text-sm">{{ $t->user->name ?? '-' }}</span>
        ...
        <!-- Low Stock Alert -->
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm shadow-gray-200/20 flex flex-col">
            <div class="flex items-center justify-between p-6 border-b border-gray-50">
                <div>
                    <h3 class="text-base font-bold text-gray-900">Peringatan Stok</h3>
                    <p class="text-xs text-gray-500">Produk hampir habis</p>
                </div>
                ...
                @if ($low_stock_products->count() > 0)
                    <div class="flex flex-col gap-4">
                        @foreach ($low_stock_products as $product)
                            <div
                                class="flex items-center justify-between p-3 rounded-xl border {{ $product->stok == 0 ? 'border-rose-100 bg-rose-50/50' : 'border-amber-100 bg-amber-50/50' }}">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 rounded-lg bg-white flex items-center justify-center shadow-sm overflow-hidden border border-gray-100">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="text-sm font-bold text-gray-900">{{ $product->nama_barang }}</span>
                                        <span
                                            class="text-[11px] font-semibold {{ $product->stok == 0 ? 'text-rose-600' : 'text-amber-600' }}">Sisa
                                            stok: {{ $product->stok }}</span>
                                    </div>
                                </div>
                                <a href="{{ route('admin.products.edit', $product->id) }}"
                                    class="text-xs font-bold text-gray-600 hover:text-emerald-600 transition-colors">Update</a>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="h-full flex flex-col items-center justify-center text-center py-6">
                        <svg class="w-12 h-12 text-emerald-200 mb-3" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <p class="text-sm font-bold text-gray-800">Semua Stok Aman</p>
                        <p class="text-xs text-gray-500 mt-1">Tidak ada produk yang perlu restock saat ini.</p>
                    </div>
                @endif
            </div>
            @if ($low_stock_count > 5)
                <div class="p-4 border-t border-gray-50 text-center">
                    <a href="{{ route('admin.products.index') }}"
                        class="text-xs font-bold text-emerald-600 hover:text-emerald-700">Lihat Semua
                        ({{ $low_stock_count }} Produk)</a>
                </div>
            @endif
        </div>
    </div>
