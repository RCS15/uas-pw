...
@section('content')
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Selamat Bekerja, Staf Kasir!</h1>
            <p class="text-sm text-gray-500">Catat transaksi penjualan hari ini secara teliti dan cepat.</p>
        </div>
        <div class="flex items-center gap-3">
            <span class="text-xs font-semibold text-teal-700 bg-teal-50 border border-teal-100 px-3.5 py-2 rounded-xl shadow-sm flex items-center gap-1.5">
                <span class="w-2.5 h-2.5 rounded-full bg-teal-500 animate-pulse"></span>
                Shift: Pagi-Siang
            </span>
            <a href="{{ route('nonadmin.transactions.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold shadow-sm shadow-emerald-600/10 transition-colors">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm shadow-gray-200/20">
            ...
            <div class="text-2xl font-bold text-gray-900">Rp 3.450.000</div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm shadow-gray-200/20">
            ...
            <div class="text-2xl font-bold text-gray-900">18 Nota</div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm shadow-gray-200/20">
            ...
            <div class="text-2xl font-bold text-gray-900">42 Pcs</div>
            <div class="mt-2 text-xs font-semibold text-gray-400">Jumlah produk terjual keluar dari gudang</div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm shadow-gray-200/20 lg:col-span-2 overflow-hidden flex flex-col justify-between">
            <div>
                <div class="flex items-center justify-between mb-6">
                    ...
                    <a href="{{ route('nonadmin.transactions.history') }}" class="text-xs font-bold text-emerald-600 hover:text-emerald-700">Lihat Semua</a>
                </div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm shadow-gray-200/20">
            ...
            <div class="space-y-3">
                @forelse ($popular_products ?? [] as $prod)
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100/50 hover:border-emerald-300 hover:bg-emerald-50/10 transition-all duration-200">
                        <div>
                            ...
                            <span class="text-xs text-emerald-700 font-semibold">Rp {{ number_format($prod['price'], 0, ',', '.') }}</span>
                        </div>
                        <a href="{{ route('nonadmin.products.show', $prod['id']) }}" class="p-2 bg-white border border-gray-200 rounded-lg text-xs font-bold text-gray-600 hover:text-emerald-700 hover:border-emerald-500 transition-colors shadow-sm">
                            Cek Stok ({{ $prod['stock'] }})
                        </a>
                    </div>
                @empty
            ...
            <a href="{{ route('nonadmin.products.index') }}" class="block text-center mt-6 text-xs font-bold text-emerald-600 hover:text-emerald-700 hover:underline">
