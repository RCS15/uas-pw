...
@section('content')
    ...
    <!-- Form Register -->
    <form action="{{ route('auth.login') }}" method="GET" class="space-y-4">
        {{-- Shop/UMKM Name Input --}}
        <div>
            <label for="umkm_name" class="block text-xs font-semibold text-gray-600 mb-1.5">Nama UMKM / Toko</label>
            <div class="relative">
                ...
                <input type="text" name="umkm_name" id="umkm_name" value="{{ old('umkm_name') }}" required
                    ...
                    placeholder="Contoh: Kopi Sedap Malam">
            </div>
            @error('umkm_name')
        ...
        {{-- Password Input --}}
        <div>
            <label for="password" class="block text-xs font-semibold text-gray-600 mb-1.5">Kata Sandi Baru</label>
            <div class="relative">
                <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-400">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
